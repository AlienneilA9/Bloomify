...
const CryptotrendsToday = ({ dir, themeName }) => {
    ...
    return (
        <Panel lg={12} xl={6} xs={12} title={t("finance_dashboard.cryptotrends_today")} subhead="Top selling items statistic by last month">
            <div dir={dir}>
                <ResponsiveContainer className="dashboard__chart-pie dashboard__chart-pie--crypto" height={360}>
                    <PieChart className="dashboard__chart-pie-container">
                        <Tooltip formatter={value => `$${value.toFixed(2)}`} position={coordinate} {...getTooltipStyles(themeName)} />
                        <Pie data={data01} dataKey="value" cy={175} innerRadius={130} outerRadius={160} label={value => `$${value.value.toFixed(2)}`} onMouseMove={onMouseMove} />
